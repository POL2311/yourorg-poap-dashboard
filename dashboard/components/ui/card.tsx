...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        // glass + radios grandes + sombra suave
        "glass rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,.35)]",
        className
      )}
      {...props}
    />
...
const CardTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3
      ref={ref}
      className={cn("text-white text-lg font-semibold leading-none tracking-tight", className)}
      {...props}
    />
  )
)
