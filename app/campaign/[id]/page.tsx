...
export default function PublicCampaignPage({ params }: { params: { id: string } }) {
  ...
  // Remaining y progreso, siempre basados en los campos del backend
  const remaining = useMemo(() => {
    if (typeof campaign?.claimsRemaining === 'number') {
      return Math.max(0, campaign.claimsRemaining)
    }
